...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Skill s1 = Skill.builder().name("Java").category("Technical").description("Core Java Programming Language & Memory Management").build();
        Skill s2 = Skill.builder().name("Spring Boot").category("Technical").description("Enterprise Microservices Framework").build();
        Skill s3 = Skill.builder().name("AWS Cloud Infrastructure").category("Technical").description("Amazon Web Services Cloud Architecture").build();
        Skill s4 = Skill.builder().name("React.js").category("Technical").description("Frontend JavaScript Library").build();
        Skill s5 = Skill.builder().name("Banking & Financial Systems").category("Domain").description("Domain expertise in core banking & fintech workflows").build();
        Skill s6 = Skill.builder().name("Healthcare Data Compliance").category("Domain").description("HIPAA and HL7 medical data standard management").build();
        Skill s7 = Skill.builder().name("Agile Leadership & Collaboration").category("Soft").description("Cross-functional sprint facilitation and team leading").build();
        Skill s8 = Skill.builder().name("Strategic Problem Solving").category("Soft").description("Enterprise analytical thinking and dispute resolution").build();

        skillRepository.saveAll(Arrays.asList(s1, s2, s3, s4, s5, s6, s7, s8));

        // 5. Seed John Smith Skills (Java 8/10, Spring Boot 7/10, etc.)
        UserSkill js1 = UserSkill.builder().userId(johnSmith.getId()).skillId(s1.getId()).skillName("Java").category("Technical").currentProficiency(80).requiredProficiency(90).ratingScore(8).level("Advanced").verified(true).build();
        UserSkill js2 = UserSkill.builder().userId(johnSmith.getId()).skillId(s2.getId()).skillName("Spring Boot").category("Technical").currentProficiency(70).requiredProficiency(85).ratingScore(7).level("Intermediate").verified(true).build();
        UserSkill js3 = UserSkill.builder().userId(johnSmith.getId()).skillId(s3.getId()).skillName("AWS Cloud Infrastructure").category("Technical").currentProficiency(85).requiredProficiency(80).ratingScore(8).level("Expert").verified(true).build();
        UserSkill js4 = UserSkill.builder().userId(johnSmith.getId()).skillId(s5.getId()).skillName("Banking & Financial Systems").category("Domain").currentProficiency(90).requiredProficiency(80).ratingScore(9).level("Expert").verified(true).build();
        UserSkill js5 = UserSkill.builder().userId(johnSmith.getId()).skillId(s7.getId()).skillName("Agile Leadership & Collaboration").category("Soft").currentProficiency(75).requiredProficiency(70).ratingScore(7).level("Intermediate").verified(true).build();

        UserSkill us1 = UserSkill.builder().userId(employee.getId()).skillId(s1.getId()).skillName("Java").category("Technical").currentProficiency(88).requiredProficiency(90).ratingScore(9).level("Advanced").verified(true).build();
        UserSkill us2 = UserSkill.builder().userId(employee.getId()).skillId(s4.getId()).skillName("React.js").category("Technical").currentProficiency(65).requiredProficiency(85).ratingScore(6).level("Intermediate").verified(false).build();
        ...
        PerformanceGoal goal1 = PerformanceGoal.builder()
                ...
                .status("In Progress")
                .dueDate("2026-09-01")
                .managerFeedback("Exceptional score on Java assessment (87%). Focus on renewing Java OCP cert.")
                .build();
        ...
        System.out.println("--> SkillSphere Platform Seed Data Created Successfully for Milestone 1!");
    }
}
